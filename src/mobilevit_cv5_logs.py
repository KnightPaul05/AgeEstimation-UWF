...
def tb_log_images(writer, tag_prefix, loader, device, max_images=8):
    ...
    # Dénormalisation simple pour affichage
    mean = torch.tensor([0.485, 0.456, 0.406], device=images.device).view(1,3,1,1)
    std  = torch.tensor([0.229, 0.224, 0.225], device=images.device).view(1,3,1,1)
    imgs_vis = (images * std + mean).clamp(0,1).cpu()
    writer.add_images(f"{tag_prefix}/images", imgs_vis, global_step=writer._global_step)
...
# ---------------------------
class CSVLogger:
    """Écrit un CSV par fold/dropout avec toutes les métriques par epoch."""
    def __init__(self, filepath):
        self.filepath = filepath
        self.fields = [
            'epoch', 'lr', 'train_loss', 'train_mae',
            'val_loss', 'val_mae', 'val_rmse', 'epoch_time_sec'
        ]
        self._ensure_header()
    ...
    def log(self, **kwargs):
        row = {k: kwargs.get(k, None) for k in self.fields}
        with open(self.filepath, 'a', newline='') as f:
            w = csv.DictWriter(f, fieldnames=self.fields)
            w.writerow(row)

def make_tb_writer(enabled, logdir):
    if not enabled:
        return None
    writer = SummaryWriter(log_dir=str(logdir))
    # petit compteur de pas pour les images
    writer._global_step = 0
    return writer
...
def main():
    ...
    for fold, (train_idx, val_idx) in enumerate(index_iter, start=1):
        ...
        for dprob in dropouts:
            ...
            # ===== NEW: loggers =====
            tb_writer = make_tb_writer(args.log_tb, fold_dir / "tb")
            csv_logger = CSVLogger(str(fold_dir / "metrics_epoch.csv")) if args.log_csv else None
            ...
            for epoch in range(1, args.epochs+1):
                ...
                if csv_logger:
                    csv_logger.log(
                        ...
                        val_mae=val_mae,
                        val_rmse=val_rmse,
                        epoch_time_sec=round(dt, 3)
                    )

                # ===== NEW: TensorBoard scalars/images =====
                if tb_writer:
                    tb_writer.add_scalar("LR", lr_now, epoch)
                    tb_writer.add_scalar("Train/Loss", tr_loss, epoch)
                    tb_writer.add_scalar("Train/MAE", tr_mae, epoch)
                    tb_writer.add_scalar("Val/Loss", val_loss, epoch)
                    tb_writer.add_scalar("Val/MAE", val_mae, epoch)
                    tb_writer.add_scalar("Val/RMSE", val_rmse, epoch)
                    if args.log_images_every > 0 and (epoch % args.log_images_every == 0):
                        tb_writer._global_step = epoch
                        tb_log_images(tb_writer, "Sample", val_loader, device)
            ...
            # Close writers for this dropout
